...
| Content-change suppression, rate control | **optional optimisation** | measured output follows the configured fps and bitrate deterministically (1 Mbit/s requests landed at 1.47-2.02 Mbit/s over 8 s including 8-9 keyframes, so a longer window is needed to state the steady-state error). Suppressing unchanged pixels on top of that would need a GPU-side signal; the native layer already exposes `AccumulatedFrames` and `LastPresentTime` |
