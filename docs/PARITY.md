...
| Rate control and keyframe policy | **needs work** | 60 s at `--fps 30 --bitrate 1000000`: fps honoured (1790 frames, 29.96 fps) but 11.66 MB = 1.55 Mbit/s, ~55% over the request. Two identical repeat runs measured 17.12 MB and 17.30 MB (**+-0.5%**), so the payload metric is repeatable; the max inter-frame gap in those same runs was 206 ms vs 422 ms, so single-sample stalls are not. A 5 s IDR period cut keyframes 61 -> 13 and raised the payload to ~17.2 MB (+48%), a real regression, and was reverted. NVENC runs CBR with `maxBitRate == averageBitRate` and a one-frame VBV buffer (`windows_nvenc.cpp`); that is the next thing to examine |
