...
| Rate control and keyframe policy | **needs work** | 60 s steady-state at `--fps 30 --bitrate 1000000`: fps honoured exactly (1790 frames, gap avg 33.38 ms = 29.96 fps) but the stream carried 11.66 MB = **1.55 Mbit/s, ~55% above the request**, with 61 keyframes (~1/s) accounting for roughly 0.27 Mbit/s on their own. The original emitted 1 keyframe in 8 s. The NVENC rate-control mode and GOP policy need review; a longer-window steady-state target should be stated explicitly |
